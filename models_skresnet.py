...
class stereo_res_unet(nn.Module):
    ...
    def forward(self, images, points, masks):
        ...
        f_image = torch.flatten(iconv2_L, 1)  # fully connected layers need 1D input
...
class stereo_skresnet(nn.Module):
    def __init__(self, num_in_layers):
        super(stereo_skresnet, self).__init__()
        # input image b x 6 x w x h (batch size, 6 channels, width, height)
        self.sigmoid = torch.nn.Sigmoid()
        # encoder
        # self.conv1_L = conv(num_in_layers, 3, 3, 2)  # H/2  -   64D
        ...
        # for skip connection
        self.adjust_conv5_L = nn.Conv2d(2048, 512, kernel_size=1, stride=1, padding=0)
        self.adaptive_pool5 = nn.AdaptiveAvgPool2d((40, 40))
        self.adjust_conv4_L = nn.Conv2d(1024, 256, kernel_size=1, stride=1, padding=0)
        self.adaptive_pool4 = nn.AdaptiveAvgPool2d((80, 80))
        self.adjust_conv3_L = nn.Conv2d(512, 128, kernel_size=1, stride=1, padding=0)
        self.adaptive_pool3 = nn.AdaptiveAvgPool2d((160, 160))
        self.adjust_conv2_L = nn.Conv2d(256, 64, kernel_size=1, stride=1, padding=0)
        self.adaptive_pool2 = nn.AdaptiveAvgPool2d((320, 320))
...
class stereo_skresnet_depth(nn.Module):
    def __init__(self, num_in_layers):
        super(stereo_skresnet_depth, self).__init__()
        # input image b x 6 x w x h (batch size, 6 channels, width, height)
        self.sigmoid = torch.nn.Sigmoid()
        # encoder
        # self.conv1_L = conv(num_in_layers, 3, 3, 2)  # H/2  -   64D
        ...
        self.depth_layers = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=3, stride=2, padding=1),  # Output: [batch_size, 32, H/2, W/2]
            nn.ReLU(),
            nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1),  # Output: [batch_size, 64, H/4, W/4]
            nn.ReLU(),
            nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1),  # Output: [batch_size, 128, H/8, W/8]
            nn.ReLU(),
            nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1),  # Output: [batch_size, 256, H/16, W/16]
            nn.ReLU(),
            nn.AdaptiveAvgPool2d((1, 1)),  # Output: [batch_size, 256, 1, 1]
    ...
    def forward(self, images, depth):
        ...
        f_image = self.fc2(f_image)
        f_image = self.dropout(f_image)
        f_image = self.fc3(f_image)

        depth_feature = self.depth_layers(depth.float())

        fuse = torch.cat((f_image, depth_feature), 1)  # concatenate image, point and mask feature
        self.left_pred = self.decoder(fuse)  # use another MLP (decoder) to predict on the fused feature

        return self.left_pred


class stereo_skresnet_axis(nn.Module):
    def __init__(self, num_in_layers):
        super(stereo_skresnet_axis, self).__init__()
        # input image b x 6 x w x h (batch size, 6 channels, width, height)
        self.sigmoid = torch.nn.Sigmoid()
        # encoder
        # self.conv1_L = conv(num_in_layers, 3, 3, 2)  # H/2  -   64D
    ...
    def forward(self, images, points):
        ...
        # points
        pt_feature = self.point_layers(points.float())

        fuse = torch.cat((f_image, pt_feature), 1)  # concatenate image, point and mask feature
        self.left_pred = self.decoder(fuse)  # use another MLP (decoder) to predict on the fused feature

        return self.left_pred


class stereo_skresnet_axis_depth(nn.Module):
    def __init__(self, num_in_layers):
        super(stereo_skresnet_axis_depth, self).__init__()
        # input image b x 6 x w x h (batch size, 6 channels, width, height)
        self.sigmoid = torch.nn.Sigmoid()
        # encoder
        # self.conv1_L = conv(num_in_layers, 3, 3, 2)  # H/2  -   64D
    ...
    def forward(self, images, points, depth):
        # input image b x 6 x w x h
        # Encoding
        x1_L = self.conv1_L(images) # encoder 1 H/2
        x_pool1_L = self.pool1_L(x1_L) # H/4
        ...
        fuse = torch.cat((f_image, pt_feature, depth_feature), 1)  # concatenate image, point and mask feature
        self.left_pred = self.decoder(fuse)  # use another MLP (decoder) to predict on the fused feature
